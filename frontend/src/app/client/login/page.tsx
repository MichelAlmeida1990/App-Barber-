...
export default function ClientLogin() {
  ...
  const handleGoogleError = (error: any) => {
    // Não exibir erro no console se for apenas cancelamento
    if (error && (error.includes('cancelado') || error.includes('access_denied'))) {
      return; // Cancelamento não é erro
    }
  ...
  const handleGoogleCancel = () => {
    // Cancelamento é uma ação válida do usuário, não precisa fazer nada
    // Apenas resetar loading se necessário
    setLoading(false);
  };
