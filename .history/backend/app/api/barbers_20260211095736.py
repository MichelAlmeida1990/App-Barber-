...
async def list_barbers(
    ...
    try:
        ...
        barbers_list = []
        for barber in db_barbers:
            barbers_list.append({
                "id": barber.id,
                "name": barber.professional_name,
                "specialties": barber.specialties or [],
                "rating": float(barber.average_rating) if barber.average_rating else 4.5,
                "experience_years": barber.experience_years or 0,
                "bio": barber.bio or "",
                "avatar_url": barber.avatar_url,
                "is_active": barber.is_active,
                "work_schedule": barber.working_hours or {}
            })
        
        # Se não houver barbeiros no banco, retornar lista mock como fallback
        if not barbers_list:
            barbers_list = [barber for barber in MOCK_BARBERS if barber["is_active"]]
        
        return {
            "barbers": barbers_list,
            "total": len(barbers_list),
            "message": "Lista de barbeiros carregada com sucesso"
        }
    except Exception as e:
        # Em caso de erro, retornar dados mock
        print(f"Erro ao buscar barbeiros do banco: {e}")
        active_barbers = [barber for barber in MOCK_BARBERS if barber["is_active"]]
        return {
            "barbers": active_barbers,
            "total": len(active_barbers),
            "message": "Lista de barbeiros carregada com sucesso (fallback)"
        }
