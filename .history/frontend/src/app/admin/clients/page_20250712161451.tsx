...
export default function ClientsPage() {
  ...
  const testAPI = async () => {
    ...
    try {
      ...
      console.log('📍 URL da API:', 'http://127.0.0.1:8000/api/v1/clients/test');
      
      const response = await clientsAPI.test();
      console.log('✅ Resposta da API:', response.data);
      
      toast.success(`✅ ${response.data.message || 'API funcionando!'}`);
      setApiStatus('✅ API conectada e funcionando');
      ...
      let errorMessage = 'Erro ao conectar com API';
      
      if (error.code === 'ERR_NETWORK') {
        errorMessage = '🚫 Erro de rede - Verifique se o backend está rodando na porta 8002';
      } else if (error.response?.status === 404) {
        errorMessage = '📍 Endpoint não encontrado - Verifique as rotas da API';
    ...
    <AdminLayout>
      <div className="pb-5 border-b border-red-200 sm:flex sm:items-center sm:justify-between mb-6 bg-gradient-to-r from-gray-50 to-red-50 rounded-lg p-6 shadow-sm border-2 border-yellow-500">
        <div className="flex items-center">
          <div className="mr-4">
            <IconFallback type="barber-chair" size="lg" className="opacity-70" />
          </div>
          <div>
            <div className="flex items-center mb-2">
              <h1 className="text-3xl font-bold leading-6 text-gray-900">CLIENTES</h1>
              <IconFallback type="comb" size="md" className="ml-3 opacity-50" />
            </div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <IconFallback type="barber-chair" size="md" className="opacity-70" />
              </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 overflow-hidden shadow-lg rounded-lg">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <IconFallback type="hair-clipper" size="md" className="opacity-70" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
