...
async def get_my_barber_appointments():
    """Listar agendamentos do barbeiro logado (da memória)"""
    
    # Por enquanto, retornar todos os agendamentos
    # Em produção, filtrar pelo barbeiro logado
    return {
        "appointments": appointments_storage,
        "total": len(appointments_storage)
    }

@router.put("/{appointment_id}/status-simple")
async def update_appointment_status_simple(appointment_id: int, status_data: dict):
    """Atualizar status de agendamento (sem autenticação para teste)"""
    
    # Buscar o agendamento na memória
    ...
    return {
        "success": True,
        "message": f"Status atualizado para {appointment['status']}",
        "appointment": appointment
    } 