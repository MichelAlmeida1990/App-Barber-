...
async def google_login(google_data: GoogleLoginRequest, db: Session = Depends(get_db)):
    ...
    # Se não encontrou por email, tentar por google_id
    ...
    if existing_user:
        ...
        if existing_user.email != email:
            # Verificar se o novo email já não está sendo usado
        ...
        # Garantir que existe registro Client para usuários CLIENT
        ...
        # Criar novo usuário com try-except para lidar com race conditions
        try:
            user = User(
                email=email,
                full_name=full_name,
            ...
            # Se falhar por UNIQUE constraint, buscar o usuário existente
            db.rollback()
            if "UNIQUE constraint failed: users.google_id" in str(e):
                # Usuário já existe, buscar por google_id
                user = db.query(User).filter(User.google_id == google_id).first()
                if not user:
                    # Se ainda não encontrou, buscar por email
                    user = db.query(User).filter(User.email == email).first()
                
                if not user:
                    # Se realmente não encontrou, re-raise o erro original
                    raise HTTPException(
