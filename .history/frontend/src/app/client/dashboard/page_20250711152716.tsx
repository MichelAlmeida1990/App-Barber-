...
export default function ClientDashboard() {
  ...
  const loadDashboardData = async (token: string) => {
    try {
      ...
      const appointmentsResponse = await fetch(
        'http://localhost:8002/api/v1/appointments/my-appointments',
        {
          headers: {
      ...
      const barbersResponse = await fetch(
        'http://localhost:8002/api/v1/barbers/',
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
      ...
      const servicesResponse = await fetch(
        'http://localhost:8002/api/v1/services/',
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const loadTimeSlots = async (barberId: number, date: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8002/api/v1/appointments/availability?barber_id=${barberId}&date=${date}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const createAppointment = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8002/api/v1/appointments/', {
        method: 'POST',
        headers: {
