...
import { MagnifyingGlassIcon, CalendarIcon, ClockIcon, UserIcon, PhoneIcon, CurrencyDollarIcon } from '@heroicons/react/24/outline';
...
export default function ConsultarAgendamento() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v1/appointments/code/${code.trim()}`);
                ...
                <div className="flex-1">
                  ...
                  {appointment.client_phone && (
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      <p className="text-gray-400 text-sm">{appointment.client_phone}</p>
                    </div>
                  )}
                </div>
              </div>
