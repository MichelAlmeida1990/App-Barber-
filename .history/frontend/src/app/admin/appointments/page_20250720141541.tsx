...
export default function AppointmentsPage() {
  ...
  const loadAppointments = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('http://127.0.0.1:8000/api/v1/appointments/admin/all');
      if (response.ok) {
        ...
        const formattedAppointments = data.map((appointment: any) => ({
          ...
          barberName: appointment.barber_name || 'Barbeiro',
          service: appointment.services?.map((s: any) => s.name).join(', ') || 'Serviço',
          date: appointment.appointment_date,
          time: appointment.time,
          duration: appointment.total_duration || 60,
          price: appointment.total_price || 0,
          status: appointment.status === 'SCHEDULED' ? 'pendente' : appointment.status?.toLowerCase() || 'pendente',
  ...
  const handleStatusChange = useCallback((appointmentId: string) => {
    setAppointments(prev => {
      ...
      if (appointment) {
        const newStatus = appointment.status === 'confirmado' ? 'pendente' : 'confirmado';
        // Mover o toast para fora do setState
        setTimeout(() => {
