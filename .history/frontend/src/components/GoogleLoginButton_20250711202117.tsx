...
export default function GoogleLoginButton({ 
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google) {
        // Configuração otimizada para evitar avisos FedCM
  ...
  const handleGoogleLogin = () => {
    ...
    try {
      if (window.google) {
        // Usar renderButton é mais estável e evita avisos
        ...
        if (buttonElement) {
          ...
          // Mostrar o botão do Google
          buttonElement.classList.remove('hidden');
          buttonElement.classList.add('mt-2');
          
          // Esconder o botão personalizado
  ...
  return (
    <div className="w-full">
      ...
      <div id="google-signin-button" className="hidden w-full"></div>
    </div>
  );
}
