...
async def get_availability(
    ...
    # Verificar se barbeiro existe (usando dados mock se necessário)
    barber = None
    try:
        barber = db.query(Barber).filter(Barber.id == barber_id).first()
    except Exception as e:
        print(f"Erro ao buscar barbeiro no banco: {e}")
        # Usar dados mock se o banco falhar
        barber = type('Barber', (), {
            'id': barber_id,
            'name': f'Barbeiro {barber_id}',
            'professional_name': f'Barbeiro {barber_id}'
        })()
    ...
    try:
        ...
        # Se houver erro no banco, usar lista vazia
    ...
    return {
        "barber_id": barber_id,
        "barber_name": getattr(barber, 'name', getattr(barber, 'professional_name', f'Barbeiro {barber_id}')),
        "date": date,
        "slots": time_slots
    }
