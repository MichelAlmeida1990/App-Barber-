...
export default function ClientDashboard() {
  ...
  const loadTimeSlots = async (barberId: number, date: string) => {
    try {
      // Usar endpoint público temporariamente
      const response = await fetch(
        `http://localhost:8000/api/v1/appointments/availability-public?barber_id=${barberId}&date=${date}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        // Em caso de erro, usar slots padrão
        const defaultSlots = generateDefaultTimeSlots();
        setTimeSlots(defaultSlots);
      }
    } catch (error) {
      console.error('Erro ao carregar horários:', error);
      // Em caso de erro de rede, usar slots padrão
      const defaultSlots = generateDefaultTimeSlots();
      setTimeSlots(defaultSlots);
    }
  };

  // Função fallback para carregar horários
  const loadTimeSlotsFallback = async (barberId: number, date: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/v1/appointments/availability-test?barber_id=${barberId}&date=${date}`,
        {
          headers: {
            'Content-Type': 'application/json'
          }
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Time slots fallback data:', data);
        setTimeSlots(data.slots || []);
      } else {
        console.error('Erro no fallback:', response.status);
        // Se tudo falhar, usar slots padrão
        const defaultSlots = generateDefaultTimeSlots();
        setTimeSlots(defaultSlots);
      }
    } catch (error) {
  ...
  // Função para gerar slots de horário padrão
