...
export default function ClientLogin() {
  ...
  const handleGoogleSuccess = async (credential: string) => {
    ...
    try {
      const response = await fetch('http://localhost:8002/api/v1/auth/google-login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Erro ao fazer login com Google');
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      {/* Background Image */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat bg-barbershop-client bg-image-loading"
        style={{
          backgroundImage: 'url(https://images.unsplash.com/photo-1521590832167-7bcbfaa6381f?w=1600&h=900&fit=crop&crop=center&auto=format&q=80)'
        }}
        onError={(e) => {
      ...
      {/* Yellow Overlay for client area theme */}
      <div className="absolute inset-0 bg-yellow-900 bg-opacity-75"></div>
      
      {/* Gradient Overlay */}
      <div className="absolute inset-0 bg-gradient-to-br from-yellow-900/60 via-transparent to-amber-900/80"></div>
