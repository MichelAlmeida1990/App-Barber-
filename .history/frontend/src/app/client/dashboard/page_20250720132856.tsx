...
export default function ClientDashboard() {
  ...
  const loadDashboardData = async (token: string) => {
    ...
    try {
      // Carregar agendamentos (usar endpoint de teste)
      const appointmentsResponse = await fetch('http://localhost:8000/api/v1/appointments/my-appointments-test', {
        headers: {
  ...
  const createAppointment = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/v1/appointments/create-simple', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
