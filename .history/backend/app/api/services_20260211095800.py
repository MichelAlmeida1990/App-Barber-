    ...
    try:
        ...
        for service in db_services:
            services_list.append({
                ...
                "category": service.category or "geral",
                "is_active": service.is_active
            })
        
        # Se não houver serviços no banco, retornar lista mock como fallback
        if not services_list:
            services_list = [service for service in MOCK_SERVICES if service["is_active"]]
            
            # Aplicar filtros no mock
            if category:
                services_list = [s for s in services_list if s["category"].lower() == category.lower()]
            if min_price is not None:
                services_list = [s for s in services_list if s["price"] >= min_price]
            if max_price is not None:
                services_list = [s for s in services_list if s["price"] <= max_price]
            
            # Ordenar por popularidade
            services_list.sort(key=lambda x: x.get("popularity", 0), reverse=True)
        ...
        # Ordenar por popularidade
        services.sort(key=lambda x: x.get("popularity", 0), reverse=True)
        ...
        return {
            ...
            "message": "Lista de serviços carregada com sucesso (fallback)"
        }
