...
async def create_appointment_simple(
    ...
    try:
        ...
        appointment = {
            ...
            "barber_id": appointment_data['barber_id'],
            "barber_name": f"Barbeiro {appointment_data['barber_id']}",
            "services": [
                {"id": service_id, "name": f"Serviço {service_id}", "price": 30.00}
                for service_id in appointment_data['service_ids']
            ],
            "appointment_date": appointment_data['appointment_date'],
            "time": appointment_data['time'],
            "status": "SCHEDULED",
            "total_price": len(appointment_data['service_ids']) * 30.00,
            "total_duration": len(appointment_data['service_ids']) * 60,
